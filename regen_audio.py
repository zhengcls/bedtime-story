...
async def main():
    ...
    tasks = []
    for ch in chapters:
        tasks.append(gen_one(ch))
    await asyncio.gather(*tasks)
    print("所有音频生成完成")

async def gen_one(ch):
    out = f"audio/ch{ch['id']}.mp3"
    # 先删除旧文件
    ...
    print(f"ch{ch['id']} done, size={size}")
...
# 验证合并结果
r2 = subprocess.run(
    ['ffprobe', '-v', 'quiet', '-print_format', 'json', '-show_streams', 'audio/full.mp3'],
    capture_output=True, text=True
)
full_dur = json.loads(r2.stdout)['streams'][0]['duration']
print(f"full.mp3 duration: {full_dur}s")

# 更新 index.html
print("\n更新 index.html...")
chapter_titles = [f"第{i+1}章" for i in range(8)]
chapter_texts = [ch["text"] for ch in chapters]

chapters_js = "[" + ",".join([
    f'{{title:"第{chapters[i]["id"]}章",text:"{chapters[i]["text"]}",start:{timestamps[i][0]},end:{timestamps[i][1]}}}'
    for i in range(8)
]) + "]"
...
<body>
    ...
    <div class="container">
        ...
        <div class="story-card">
            ...
            <div class="chapter-text" id="chapterText">{chapters[0]["text"]}</div>
        </div>
...
print("\nGit 推送...")
os.system('git add -A')
os.system('git commit -m "fix: 重新生成音频和时间戳" --quiet')
        ...
        "elements": [
            ...
            {"tag": "action", "actions": [
                {"tag": "button", "text": {"tag": "plain_text", "content": "点击收听"},
                 "type": "primary", "url": "https://zhengcls.github.io/bedtime-story/"}
            ]}
        ]
    }
}
